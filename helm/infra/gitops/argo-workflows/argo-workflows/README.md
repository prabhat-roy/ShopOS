...
| crds.upgradeJob | object | `{"crdBaseURL":"","extraEnv":[],"hostPath":"","image":{"repository":"registry.k8s.io/kubectl","tag":"v1.36.0"},"imagePullSecrets":[],"nodeSelector":{},"resources":{},"tolerations":[]}` | Configuration for the CRD install Job (only used when crds.full=true) |
...
| crds.upgradeJob.image | object | `{"repository":"registry.k8s.io/kubectl","tag":"v1.36.0"}` | Image for the kubectl container that applies CRDs |
| crds.upgradeJob.image.repository | string | `"registry.k8s.io/kubectl"` | Repository for the kubectl image |
| crds.upgradeJob.image.tag | string | `"v1.36.0"` | Tag for the kubectl image |
