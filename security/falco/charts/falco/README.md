...
Note that since Falco 0.36.0 and Helm chart version 3.7.0 the driver loader image has been updated to be compatible with newer kernels (5.x and above) meaning that if you have an older kernel version and you are trying to build the kernel module you may experience issues.
In that case you can use an alternative `falco-driver-loader` image flavor (e.g. `falcosecurity/falco-driver-loader:latest-buster`) to use an older version of the toolchain. To do so you can set the appropriate value, i.e. `--set driver.loader.initContainer.image.tag=latest-buster`.
...
> âš ï¸ DEPRECATION NOTICE: The gVisor engine (`driver.kind=gvisor`) is deprecated starting with Falco 0.43.0. Consider using alternative monitoring approaches. See [BREAKING-CHANGES.md](./BREAKING-CHANGES.md) for more information.
...
> âš ï¸ DEPRECATION NOTICE: The Legacy eBPF probe (`driver.kind=ebpf`) is deprecated starting with Falco 0.43.0. Please consider using the Modern eBPF probe (`driver.kind=modern_ebpf`) instead. See [BREAKING-CHANGES.md](./BREAKING-CHANGES.md) for more information.
...
The following table lists the main configurable parameters of the falco chart v8.0.2 and their default values. See [values.yaml](./values.yaml) for full list.
...
| falco.plugins_hostinfo | bool | `true` | Control host info support for plugins. When `false`, it disables host info support for source plugins that DO NOT generate raw events from the libscap event table or for plugins that DO NOT parse raw events generated by drivers, effectively dropping the `proc-fs` hostPath volume requirement for them: https://github.com/falcosecurity/charts/blob/bd57711e7c8e00919ea288716e0d9d5fdad8867e/charts/falco/templates/pod-template.tpl#L302-L304 |
