...
Coupling these reactions synchronously into the checkout flow would increase latency by 500–2000ms and create hard dependency chains where a single downstream failure blocks checkout.
...
- Topic naming convention: `{domain}.{entity}.{event}` — e.g., `commerce.order.placed`
...
1. **Durability** — Kafka persists events to disk. Consumers that go down can replay from any offset.
2. **Throughput** — Handles millions of events/second; analytics and fraud detection process high-volume streams without backpressure on producers.
3. **Decoupling** — Producers have no knowledge of consumers. Adding a new consumer requires zero producer changes.
4. **Event sourcing** — The event-store-service uses Kafka as an append-only log, enabling full projection rebuilds and audit trails.
5. **Stream processing** — Apache Flink consumes Kafka topics for real-time fraud detection and analytics aggregations.
