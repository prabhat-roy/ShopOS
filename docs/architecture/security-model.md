...
| Control | Status |
|---|---|
| Zero-trust network | ✓ Istio mTLS + Cilium default-deny |
| Workload identity | ✓ SPIFFE/SPIRE SVIDs, hourly rotation |
| No hardcoded secrets | ✓ Vault dynamic secrets + ESO |
| Image signing enforced | ✓ Cosign + Kyverno admission check |
| SBOM for every image | ✓ Syft CycloneDX at build time |
| Runtime threat detection | ✓ Falco + Tetragon + Wazuh SIEM |
| Policy-as-code | ✓ OPA + Kyverno + Kubewarden |
| Compliance | ✓ GDPR, PCI-DSS (Wazuh dashboards) |
