def call() {
    sh """
        helm upgrade --install kafka-ui messaging/kafka-ui/charts \
            ...
            --set env.KAFKA_CLUSTERS_0_BOOTSTRAPSERVERS=kafka-kafka.kafka.svc.cluster.local:9092 \
            --set env.KAFKA_CLUSTERS_0_SCHEMAREGISTRY=http://schema-registry-schema-registry.schema-registry.svc.cluster.local:8081 \
            --set env.KAFKA_CLUSTERS_0_KAFKACONNECT_0_NAME=connect \
            --set env.KAFKA_CLUSTERS_0_KAFKACONNECT_0_ADDRESS=http://kafka-connect-kafka-connect.kafka-connect.svc.cluster.local:8083 \
            --wait --timeout 5m
    """
