pipeline {
    ...
    parameters {
        ...
        booleanParam(name: 'PULSAR',          defaultValue: false, description: 'Apache Pulsar — multi-tenant messaging and streaming')
        booleanParam(name: 'ACTIVEMQ',        defaultValue: false, description: 'ActiveMQ Artemis — enterprise message broker')
        booleanParam(name: 'REDPANDA',        defaultValue: false, description: 'Redpanda — Kafka-compatible high-performance broker')
        booleanParam(name: 'MEMPHIS',         defaultValue: false, description: 'Memphis — developer-friendly message broker')
