...
import (
	...
	"github.com/shopos/recently-viewed-service/internal/config"
...
func main() {
	logger := log.New(os.Stdout, "[recently-viewed-service] ", log.LstdFlags|log.Lshortfile)
	...
	st := store.New(rdb)
	svc := service.New(st, cfg.MaxItems, cfg.ItemTTL)
	h := handler.New(svc)
